                ...
                <div class="table-responsive">
                    ...
                    <table class="table table-bordered table-hover singcms-table">
                        ...
                        <?php if(is_array($admins)): $i = 0; $__LIST__ = $admins;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$admin): $mod = ($i % 2 );++$i;?><tr>
                                ...
                                <td><?php echo (date("Y-m-d H:i",$admin["lastlogintime"])); ?></td>
                                <td><span  attr-status="<?php if($admin['status'] == 1): ?>0<?php else: ?>1<?php endif; ?>"  attr-id="<?php echo ($admin["admin_id"]); ?>" class="sing_cursor singcms-on-off" id="singcms-on-off" ><?php echo (status($admin["status"])); ?></span></td>
                                <td><a href="javascript:void(0)" attr-id="<?php echo ($admin["admin_id"]); ?>" id="singcms-delete"  attr-a="admin" attr-message="删除"><span class="glyphicon glyphicon-remove-circle" aria-hidden="true"></span></a></td>
