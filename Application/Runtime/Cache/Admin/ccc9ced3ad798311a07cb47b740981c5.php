          ...
          <div class="col-md-3">
            <div class="input-group">
              ...
              <select class="form-control" name="catid">
                <option value='' <?php if(1 == 1): ?>selected="selected"<?php endif; ?>>全部分类</option>
                <?php if(is_array($webSiteMenu)): foreach($webSiteMenu as $key=>$sitenav): ?><option value="<?php echo ($sitenav["menu_id"]); ?>" <?php if($MenuId == $sitenav['menu_id']): ?>selected="selected"<?php endif; ?>><?php echo ($sitenav["name"]); ?></option><?php endforeach; endif; ?>
              </select>
            </div>
          </div>
      ...
      <div class="row">
        <div class="col-lg-12">
          ...
          <div class="table-responsive">
            <form id="singcms-listorder" method="get" action="admin.php">
              <table class="table table-bordered table-hover singcms-table">
                ...
                <?php if(is_array($news)): $i = 0; $__LIST__ = $news;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$new): $mod = ($i % 2 );++$i;?><tr>
                    ...
                    <td><span class="sing_cursor glyphicon glyphicon-edit" aria-hidden="true" id="singcms-edit" attr-id="<?php echo ($new["news_id"]); ?>" ></span>
                      <a href="javascript:void(0)" id="singcms-delete"  attr-id="<?php echo ($new["news_id"]); ?>"  attr-a="news" attr-message="删除">
                        <span class="glyphicon glyphicon-remove-circle" aria-hidden="true"></span>
                      </a>
                      <a href="admin.php?m=home&c=Detail&a=view&id=<?php echo ($new["news_id"]); ?>">
            ...
            <div class = "input-group">
              <select class="form-control" name="position_id" id="select-push1">
                <option>请选择推荐位进行推送</option>
                <?php if(is_array($positions)): foreach($positions as $key=>$position): ?><option value="<?php echo ($position["id"]); ?>" id="rehellinen"><?php echo ($position["name"]); ?></option><?php endforeach; endif; ?>
              </select>
            </div>
            <br />
            <div>
